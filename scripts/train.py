...
TensorRun_name= "baseline A03"
zone_centers = np.load(str(base_dir / "maps" / f"{map_name}{run_name}.npy"))
...
save_dir = base_dir / "save" / TensorRun_name
save_dir.mkdir(parents=True, exist_ok=True)
tensorboard_writer = SummaryWriter(log_dir=str(base_dir / "tensorboard" / TensorRun_name))
...
trainer = iqn.Trainer(
    ...
    epsilon_decay = misc.epsilon_decay,  
    min_epsilon = misc.min_epsilon,  
...
for loop_number in count(1):
    ...
    if is_explo:
        print("exploring")
        print(f"Total training time: {accumulated_stats['cumul_training_hours']:.2f} hours")
        trainer.epsilon = (
        ...
        print("Validating training session (testing learned exploring)" )
        print(f"Total training time: {accumulated_stats['cumul_training_hours']:.2f} hours")
    ...
    if end_race_stats["race_finished"]:
        print("Time to finish: ", end_race_stats["race_time"]/1000)
        print(f"Total training time: {accumulated_stats['cumul_training_hours']:.2f} hours")
