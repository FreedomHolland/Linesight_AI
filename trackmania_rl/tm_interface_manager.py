...
def _get_window_position():
    ...
    left = rect[0] + round(((rect[2] - rect[0]) - misc.W_screen) / 2)
    output_idx = 0
...
class TMInterfaceManager:
    def __init__(self, base_dir, running_speed=1, run_steps_per_action=10, max_overall_duration_ms=2000, max_minirace_duration_ms=2000, interface_name="TMInterface0", zone_centers=None):
        self.iface = None
        ...
        self.pinned_buffer_size = misc.memory_size + 100
        self.pinned_buffer = torch.empty((self.pinned_buffer_size, 1, misc.H_downsized, misc.W_downsized), dtype=torch.uint8)
        torch.cuda.cudart().cudaHostRegister(self.pinned_buffer.data_ptr(), self.pinned_buffer_size * misc.H_downsized * misc.W_downsized, 0)
    ...
    def rollout(self, exploration_policy, is_eval):
        ...
        zone_centers_delta[:, 1] *= 0.1
        zone_centers_delta[-(3 + misc.n_zone_centers_in_inputs):, :] = 0
        if is_eval:
        ...
        rollout_results["zone_entrance_time_ms"].append(0)
        ...
        while not (this_rollout_is_finished and time.perf_counter_ns() > do_not_exit_main_loop_before_time):
            ...
            ignore_message0 = ((msgtype & 0xFF) == 0) and prev_msgtype == 0 and (time.perf_counter_ns() > time_first_message0 + 1000_000_000)
            ...
            if (msgtype & 0xFF != 14) and (((msgtype & 0xFF00) == 0) or ignore_message0):
                ...
                if compute_action_asap and give_up_signal_has_been_sent and this_rollout_has_seen_t_negative and not this_rollout_is_finished and time.perf_counter_ns() > do_not_compute_action_before_time:
                    ...
                    if current_zone_idx == len(zone_centers) - 1 - misc.n_zone_centers_in_inputs:
                        ...
                        sim_state_position = np.array(last_known_simulation_state.dyna.current_state.position, dtype=np.float32)
                        sim_state_orientation = last_known_simulation_state.dyna.current_state.rotation.to_numpy()
                        sim_state_velocity = np.array(last_known_simulation_state.dyna.current_state.linear_speed, dtype=np.float32)
                        sim_state_angular_speed = np.array(last_known_simulation_state.dyna.current_state.angular_speed, dtype=np.float32)
                        sim_state_car_gear_and_wheels = np.array([
                            ...
                            last_known_simulation_state.scene_mobil.gearbox_state,
                            last_known_simulation_state.scene_mobil.engine.gear,
                        ...
                        if d1 <= d2 and d1 <= misc.max_allowable_distance_to_checkpoint and current_zone_idx < len(zone_centers) - 2 - misc.n_zone_centers_in_inputs:
                            rollout_results["fraction_time_in_previous_zone"].append(
                                fraction_time_spent_in_current_zone(current_zone_center, next_zone_center, prev_sim_state_position, sim_state_position)
                            )
                            next_zone_center = zone_centers[1 + current_zone_idx]
                            previous_zone_center = zone_centers[current_zone_idx - 1] if current_zone_idx > 0 else (2 * zone_centers[0] - zone_centers[1])
                            ...
                            rollout_results["fraction_time_in_previous_zone"].append(np.nan)
                        ...
                        meters_in_current_zone = np.clip((sim_state_position - pointA).dot(pointB - pointA) / dist_pointB_pointA, 0, dist_pointB_pointA)

                        # print("current zone: ", current_zone_center, "next zone: ", next_zone_center)
                        ...
                        frame = torch.from_numpy(np.expand_dims(cv2.resize(cv2.cvtColor(frame, cv2.COLOR_BGRA2GRAY), (misc.W_downsized, misc.H_downsized), interpolation=cv2.INTER_AREA), 0))
                        ...
                        state_zone_center_coordinates_in_car_reference_system = sim_state_orientation.T.dot((zone_centers[current_zone_idx:current_zone_idx + misc.n_zone_centers_in_inputs, :] - sim_state_position).T).T
                        ...
                        floats = np.hstack((0, np.array([
                            previous_action["accelerate"], previous_action["brake"], previous_action["left"], previous_action["right"]
                        ]), sim_state_car_gear_and_wheels.ravel(), state_car_angular_velocity_in_car_reference_system.ravel(), state_car_velocity_in_car_reference_system.ravel(), state_y_map_vector_in_car_reference_system.ravel(), state_zone_center_coordinates_in_car_reference_system.ravel())).astype(np.float32)

                        time_A_stack += time.perf_counter_ns() - pc2
                        pc2 = time.perf_counter_ns()

                        action_idx, action_was_greedy, q_value, q_values = exploration_policy(rollout_results["frames"][-1], floats)
            ...
            if msgtype == MessageType.S_SHUTDOWN:
                ...
                if (_time > self.max_overall_duration_ms or _time > rollout_results["zone_entrance_time_ms"][-1]+ self.max_minirace_duration_ms) and this_rollout_has_seen_t_negative and not this_rollout_is_finished:
                ...
                if not this_rollout_is_finished:
                    ...
                    elif _time >= 0 and this_rollout_has_seen_t_negative and self.latest_tm_engine_speed_requested == 0:
                ...
                if current == target:
                    ...
                    if this_rollout_has_seen_t_negative and not this_rollout_is_finished:
                        ...
                        if rollout_results["current_zone_idx"][-1] != len(zone_centers) - 1 - misc.n_zone_centers_in_inputs:
                            ...
                            rollout_results["fraction_time_in_previous_zone"].append(
                                (simulation_state.race_time - (len(rollout_results["fraction_time_in_previous_zone"]) - 1) * misc.ms_per_action) / misc.ms_per_action
                            )

                            temp_sim_state_position = np.array(last_known_simulation_state.dyna.current_state.position, dtype=np.float32)
                            temp_sim_state_velocity = np.array(last_known_simulation_state.dyna.current_state.linear_speed, dtype=np.float32)
                            meters_in_current_zone = (temp_sim_state_position + (1 - rollout_results["fraction_time_in_previous_zone"][-1]) * temp_sim_state_velocity - pointA).dot(pointB - pointA) / dist_pointB_pointA

                            assert meters_in_current_zone >= 0.8 * dist_pointB_pointA
                            rollout_results["meters_advanced_along_centerline"].append(prev_zones_cumulative_distance + meters_in_current_zone)
