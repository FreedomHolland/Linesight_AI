...
class Agent(torch.nn.Module):
    ...
    def forward(
        ...
        img_outputs = self.img_head((img.to(torch.float32 if use_fp32 else torch.float16) - 128) / 128)  
        ...
        with torch.cuda.stream(self.execution_stream):
            with torch.amp.autocast(device_type="cuda", dtype=torch.float16):
                with torch.no_grad():
                    ...
                    done = new_done.reshape(-1, 1).repeat([self.iqn_n, 1])  
                    actions = new_actions[:, None]  
                    actions_n = actions.repeat([self.iqn_n, 1])  
            ...
            with torch.amp.autocast(device_type="cuda", dtype=torch.float16):
                ...
                tau3 = tau3.reshape([self.iqn_n, self.batch_size, 1]).transpose(0, 1)  
                tau3 = tau3[:, None, :, :].expand([-1, self.iqn_n, -1, -1])  
                loss = (
